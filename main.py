            ...
            if llList[k] == subsetindex:  # check in right order
                ...
                for d in c.iter('LigatureSet'):  # effectively search on for type 4 subst
                    ...
                    for e in d.iter('Ligature'):
                        ...
                        if (forglyph == 'ud730'):  # for debugging a particular char
...
def retrieve_input():
    ...
    for ii in range(0, len(inputValue) - 3):  # skip last 3 padded spaces
        ...
        uniAppend = hex(ord(inputValue[ii]))+","
        ...
        # bypass 3 chars for triple substitution, etc.
        if level3:
            level3 = False
            continue
        ...
        if not level2:  # do following only if pre-positioning is not already done
            ...
            for ijk in range(0, len(substList)):
                ...
                if glyID == substList[ijk][0]:  # check if current char is in subst list
                    ...
                    nextComplen = len(substList[ijk][1])
                    ...
                    # print("yes, current char is in substList and nextcomp = ", nextComp)

                    if nextComplen == 3:   # [0]+(1,2,3) --> subst, 3 component subst.
                        if glyID2 == int(nextComp[0]):
                            if glyID3 == int(nextComp[1]):
                                if glyID4 == int(nextComp[2]):
                                    substglyID = substList[ijk][2]
                                    # print("found in subst level 4", hex(glyID), hex(glyID2), hex(glyID3),nextComp)
                                    charAppend = "g+" + str(hex(substglyID)).replace("0x", "")
                                    level2 = True
                                    level3 = True
                                    level4 = True
                                    finalDisp = finalDisp + charAppend
                    ...
                    if nextComplen == 2:  # [0]+(1,2) --> s1, then [s1]+(3) --> subst
                        if glyID2 == int(nextComp[0]):
                            if glyID3 == int(nextComp[1]):
                                substglyID = substList[ijk][2]  # [0]+(1,2) --> s1 done
                                # print("found in subst level 3", hex(glyID), hex(glyID2), hex(glyID3), nextComp)
                                # charAppend = "g+" + str(hex(substglyID)).replace("0x", "")

                                for ijkl in range(0, len(substList)):
                                    if substglyID == substList[ijkl][0]:
                                        nextnextComp = str(substList[ijkl][1])
                                        nextnextComp = nextnextComp.replace("[", "")
                                        nextnextComp = nextnextComp.replace("]", "")
                                        nextnextComp = nextnextComp.split(",")
                                        # ("nxtnxt 3,1", nextnextComp)
                                        if glyID4 == int(nextnextComp[0]):
                                            substsubstglyID = substList[ijkl][2]  # [s1]+(3) --> subst here
                                            # print("subst level 3,1", hex(glyID), hex(glyID2), hex(glyID3), nextComp)
                                            charAppend = "g+" + str(hex(substsubstglyID)).replace("0x", "")
                                            level2 = True
                                            level3 = True
                                            level4 = True
                                            finalDisp = finalDisp + charAppend
                    ...
                    if not level4:  # do if previous two subst are not done

                        if nextComplen == 2:   # [0]+(1,2) --> subst
                            if glyID2 == int(nextComp[0]):
                                if glyID3 == int(nextComp[1]):
                                    substglyID = substList[ijk][2]  # [0]+(1,2) --> subst
                                    # print("subst level 2", hex(glyID), hex(glyID2), hex(glyID3),nextComp)
                                    charAppend = "g+" + str(hex(substglyID)).replace("0x", "")
                                    level2 = True
                                    level3 = True
                                    # finalDisp = finalDisp + charAppend
                    ...
                    if not level3:  # do this if [0]+(1,2) --> subst is not done

                        if nextComplen == 1:   # [0]+(1) --> s1, [s1]+(2) --> subst
                            if glyID2 == int(nextComp[0]):
                                substglyID = substList[ijk][2]
                                # print("subst level 2", hex(glyID), hex(glyID2), hex(glyID3), nextComp)
                                for ijkl in range(0, len(substList)):
                                    if substglyID == substList[ijkl][0]:
                                        ...
                                        nextnextComp = nextnextComp.replace("]", "")
                                        nextnextComp = nextnextComp.split(",")
                                        # print("nxtnxt 3,1", nextnextComp)
                                        if glyID3 == int(nextnextComp[0]):
                                            substsubstglyID = substList[ijkl][2]
                                            # print("subst level 2,1", hex(glyID), hex(glyID2), hex(glyID3), nextComp)
                                            charAppend = "g+" + str(hex(substsubstglyID)).replace("0x", "")
                                            level2 = True
                                            level3 = True
                                            # finalDisp = finalDisp + charAppend
                    ...
                    if not level2:  # do this previous three subst not done

                        if nextComplen == 1:   # [0]+(1) --> subst
                            if glyID2 == int(nextComp[0]):
                                substglyID = substList[ijk][2]
                                # print("subst level 1", hex(glyID), hex(glyID2), hex(glyID3), nextComp)
                                charAppend = "g+" + str(hex(substglyID)).replace("0x", "")
                                level2 = True
                                # finalDisp = finalDisp + charAppend
                                # continue
